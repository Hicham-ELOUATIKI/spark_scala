...
object Preprocessor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(show_outputs) dfCasted.describe().show
    ...
    // cette colonne est très largement majoritairement à "false", il y a 322 "true" (négligable) le reste est non-identifié donc:
    ...
    // Version 2 de udfCountry avec mattern matching
    ...
    if(show_outputs) dfLower.take(50)
    ...
    if(show_outputs) dfReady.show(50)
    if(show_outputs) println(dfReady.count)

    /** 5 - WRITING DATAFRAME **/
    dfReady
      .write
      .mode(SaveMode.Overwrite)
      .parquet(  dataDir + "/prepared_trainingset_hicham")

      println("\n")
      println("Fin d'execution ...............")
      println("\n")
  }
}
